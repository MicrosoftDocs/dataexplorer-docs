...
:::moniker-end
:::moniker range="microsoft-fabric"
* If the eventhouse or databases are in different tenants, follow the instructions in [Allow cross-tenant queries and commands](/azure/data-explorer/access-control/cross-tenant-query-and-commands).
:::moniker-end
...
|Real-time intelligence |The default database is the current database selected either [directly](/fabric/real-time-intelligence/access-database-copy-uri) or through an eventhouse.|
...
:::moniker range="microsoft-fabric"
* Any entity name starting with *my...* in the default database.
* Any table in all the databases named *MyOther...* of the current eventhouse.
* Any table in all the databases named *my2...* in the eventhouse *OtherEventhouse.kusto.windows.net*.

```kusto
restrict access to (my*, database("MyOther*").*, cluster("OtherEventhouse").database("my2*").*);
```
:::moniker-end
...
```kusto
let MyView = Table1 join database("OtherDb").Table2 on Key | join cluster("OtherEventhouse").database("SomeDb").Table3 on Key;
MyView | where ...
```
...
```kusto
cluster("OtherEventhouse").database("SomeDb").MyView("exception") | count
```
