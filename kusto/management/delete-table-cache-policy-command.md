...
Delete the table's cache policy. To speed up queries on data, the query environment caches it on its processing nodes, SSD, or even in RAM. The [cache policy](cache-policy.md) lets the query environment describe the data artifacts that it uses so that important data can take priority.
