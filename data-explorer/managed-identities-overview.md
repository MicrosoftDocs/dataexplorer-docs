...
* **Ingest from storage**: Retrieve data from multiple files in Azure Blob Storage and ingest it into a single target table using managed identity authentication. Refer to the [ingestion from storage](kusto/management/data-ingestion/ingest-from-storage.md) documentation for examples.
