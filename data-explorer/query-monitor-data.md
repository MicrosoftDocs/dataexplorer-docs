...
> * If names include characters that don't meet [KQL identifier name rules](/kusto/query/schema-entities/entity-names), they are replaced by the dash **-** character.
...
> [!TIP]
> Using the [`join` operator](/kusto/query/join-operator), instead of union, may require a [`hint`](/kusto/query/join-operator#hints) to run it on an Azure Data Explorer native cluster.
