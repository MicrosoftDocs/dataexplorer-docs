...
| [**ADX Kafka**](ingest-data-kafka.md) |Avro, ApacheAvro, JSON, CSV, Parquet, and ORC |Unlimited. Inherits Java restrictions.| Batching, streaming |Either already have a Kafka setup and want to simply add Kusto as an additional sink without writing code, or prefer the mature, open source nature of Kafka for high volume consumption from the source over reinventing the wheel with your own code sourcing data (especially from multiple sources) to feed into a Kusto SDK.| The Kafka + Kusto Kafka Connector stack is conceptually similar to the Event Hub + Kusto stack, with preference often determined by which “multiple producer/consumer” service is already used, or how managed of a service is desired. |
| [**ADX to Apache Spark**](spark-connector.md) | Every format supported by the Spark environment  | Unlimited | Batching | Either already have the data on Spark, want to do some preprocessing on Spark before ingestion or want a fast way to create a safe (Spark) streaming pipeline from the various sources the Spark environment supports. | If data is not already in Spark native queued ingest might be cheaper and faster way as the cost of the Spark cluster should also be taken into consideration. When considering batch write - one might want to compare this with ADX data connection for Event Grid, for Spark streaming - compare with the data connection for Event Hub.
| [**LogStash**](ingest-data-logstash.md) | JSON | Unlimited. Inherits Java restrictions. | Inputs to the connector are Logstash events, and the connector outputs to Kusto using batching ingestion.|  Either already have a Logstash setup and want to simply add Kusto as an additional output without writing code, or prefer the mature, open source nature of Logstash for high volume consumption from the input(s) over reinventing the wheel with your own code sourcing data (especially from multiple sources) to feed into a Kusto SDK.| The Logstash + Kusto Logstash Connector stack is conceptually similar to the Event Hub + Kusto stack, with preference often determined by which “multiple producer/consumer” service is already used, or how managed of a service is desired.
