...
* If streaming ingestion is enabled on a cluster used as a leader for [follower databases](follower.md), streaming ingestion must be enabled on the following clusters as well to follow streaming ingestion data.
* If streaming ingestion is used on any of the tables of the database, this database cannot be used as a [data provider](data-share.md#data-provider---share-data) for Azure Data Share.
