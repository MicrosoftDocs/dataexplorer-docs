...
1. Enter a [storage connection strings](/kusto/api/connection-strings/storage-connection-strings) to your source container with read and list permissions. When verification has completed, a green check will appear to the right of the container link.
...
In the right-hand side of the tab, you can preview your data. On the left-hand side, you can add [partitions](/kusto/management/partitioning-policy) to your table definitions to access the source data more quickly and achieve better performance.

> [!NOTE]
> Mappings are not part of the definition of an external table, and are not supported in this wizard. Mappings can be [configured later](/kusto/management/external-table-mapping-create) if necessary. Some functionalities, such as deleting the last column in CSV files or changing column names in JSON files, require mappings in order to work correctly.
