...
If you want to secure outbound traffic using [Azure Firewall](/azure/firewall/overview) or any virtual appliance to limit domain names, allow in the firewall all Fully Qualified Domain Names (FQDN) from the [up-to-date list of required outbound network dependencies](vnet-deploy-secure-outbound-traffic-firewall.md).
