...
The script can only run control commands that start with the following verbs:
...
Those commands must apply to the database and must therefore be "database level" (e.g. `.create-or-alter table`) as opposed to "cluster level" (e.g. `.alter cluster` policies).
