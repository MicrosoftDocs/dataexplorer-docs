...
There are various methods you can use to configure a database with Kusto Query Language scripts. We'll focus on two main methods using ARM template deployment. In the first, [**simple** method](#simple-method-upload-kusto-query-language-script), you create a script as a blob in an Azure storage account, and provide its details (url and [shared access signatures (SaS)](/azure/storage/common/storage-sas-overview)) directly. In the second, [more advanced scenario](#advanced-method-inline-kusto-query-language-script), you provide your Kusto Query Language script **inline**, and a storage account is created during the deployment.
