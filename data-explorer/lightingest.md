...
1. Enter `ingest-` followed by the connection string to the Azure Data Explorer cluster that will manage the ingestion. Enclose the connection string in double quotes and follow the [Kusto connection strings specification](/kusto/api/connection-strings/kusto).
...
|----------|------|-------------|----------|
|          | `string` | A [Kusto connection string](/kusto/api/connection-strings/kusto) specifying the Kusto endpoint that handles the ingestion. This value should be enclosed with double quotes. |  :heavy_check_mark: |
...
| -ingestionMappingPath, -mappingPath | `string` | A path to a local file for ingestion column mapping. See [data mappings](/kusto/management/mappings). |  |
| -ingestionMappingRef, -mappingRef | `string` | The name of an ingestion column mapping that was previously created on the table. See [data mappings](/kusto/management/mappings). |  |
...
| -tag | `string` | [Tags](/kusto/management/extent-tags) to associate with the ingested data. Multiple occurrences are permitted |  |
...
> [!IMPORTANT]
> When specifying that the creation time should be overridden, make sure the `Lookback` property in the target table's effective [Extents merge policy](/kusto/management/merge-policy) is aligned with the values in your file or blob paths.
