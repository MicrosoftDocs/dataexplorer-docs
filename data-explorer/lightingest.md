...
1. Enter `ingest-` followed by the connection string to the Azure Data Explorer cluster that manages the ingestion. Enclose the connection string in double quotes and follow the [Kusto connection strings specification](/kusto/api/connection-strings/kusto?view=azure-data-explorer&preserve-view=true).
...
* For optimal ingestion performance, LightIngest needs the raw data size so it can estimate the uncompressed size of local files. However, LightIngest might not be able to correctly estimate the raw size of compressed blobs without first downloading them. Therefore, when ingesting compressed blobs, set the `rawSizeBytes` property on the blob metadata to uncompressed data size in bytes.

## Command-line arguments

| Argument | Type | Description | Required |
|----------|------|-------------|----------|
|          | `string` | A [Kusto connection string](/kusto/api/connection-strings/kusto?view=azure-data-explorer&preserve-view=true) specifying the Kusto endpoint that handles the ingestion. Enclose this value in double quotes. |  :heavy_check_mark: |
...
| -azCli | `bool` | If set, uses the Azure CLI to authenticate to the Kusto service. The Azure CLI must be installed and signed in. |  |
...
| -connectToStorageLoginUri, -storageLoginUri | `string` | If `-connectToStorageWithUserAuth` is set, you can optionally provide a Microsoft Entra ID sign-in URI. |  |
| -prefix | `string` | When the source data to ingest resides on blob storage, this URL prefix is shared by all blobs, excluding the container name. <br>For example, if the data is in `MyContainer/Dir1/Dir2`, then the prefix should be `Dir1/Dir2`. Enclose this value in double quotes. |  |
| -pattern | `string` | Pattern by which source files and blobs are picked. Supports wildcards. For example, `"*.csv"`. Enclose this value in double quotes. |  |
| -zipPattern | `string` | Regular expression to use when selecting which files in a ZIP archive to ingest. All other files in the archive are ignored. For example, `"*.csv"`. Enclose this value in double quotes. |  |
...
| -ingestionMappingRef, -mappingRef | `string` | The name of an ingestion column mapping that you previously created on the table. See [data mappings](/kusto/management/mappings?view=azure-data-explorer&preserve-view=true). |  |
...
| -ignoreFirstRow, -ignoreFirst | `bool` | If set, the first record of each file or blob is ignored. For example, if the source data has headers. |  |
| -tag | `string` | [Tags](/kusto/management/extent-tags?view=azure-data-explorer&preserve-view=true) to associate with the ingested data. Multiple occurrences are permitted |  |
| -dontWait | `bool` | If set to `true`, doesn't wait for ingestion completion. Useful when ingesting large amounts of files and blobs. |  |
| -compression, -cr | double | Compression ratio hint. Useful when ingesting compressed files and blobs to help Azure Data Explorer assess the raw data size. Calculated as original size divided by compressed size. |  |
| -limit, -l | integer | If set, limits the ingestion to first *N* files. |  |
| -listOnly, -list | `bool` | If set, only displays the items that would be selected for ingestion. |  |
...
| -filesInBatch | integer | Sets the file and blob count limit of each ingest operation. |  |
...
When you use LightIngest with Azure blobs, it uses certain blob metadata properties to enhance the ingestion process.

|Metadata property                            | Usage                                                                           |
|---------------------------------------------|---------------------------------------------------------------------------------|
|`rawSizeBytes`, `kustoUncompressedSizeBytes` | If set, LightIngest interprets these properties as the uncompressed data size   |
|`kustoCreationTime`, `kustoCreationTimeUtc`  | LightIngest interprets these properties as UTC timestamp. If set, it uses these properties to override the creation time in Kusto. This feature is useful for backfilling scenarios |

## Usage examples

The following examples assume you installed LightIngest binaries for your operating system. If you installed LightIngest as a .NET tool, substitute `LightIngest` with `LightIngest` in the examples.

### Ingest historical data with the CreationTime property

When you load historical data from an existing system to Azure Data Explorer, all records get the same ingestion date. To partition your data by creation time instead of ingestion time, use the `-creationTimePattern` argument. The `-creationTimePattern` argument extracts the `CreationTime` property from the file or blob path. The pattern doesn't need to reflect the entire item path, just the section that encloses the timestamp you want to use.
...
### Ingesting blobs by using a storage account key or a SAS token
...
* Ingest all files under path `PATH` that match the pattern `*.json`
* Set the destination to database `DB`, table `TABLE`, and define the ingestion mapping in local file `MAPPING_FILE_PATH`
* The tool posts the data for ingestion and doesn't wait for the ingest operations to complete
...
### Ingest files and write diagnostic trace files

* Ingest all files under path `PATH` that match the pattern `*.json`
* Set the destination to database `DB`, table `TABLE`, and define the ingestion mapping in local file `MAPPING_FILE_PATH`
* The tool posts the data for ingestion and doesn't wait for the ingest operations to complete
* Write diagnostics trace files locally under folder `LOGS_PATH`
...
LightIngest performs three actions that can use managed identity for authentication. The use of managed identity in each step doesn't require use of managed identity in other steps. For each action, the related [command-line argument](#command-line-arguments) is given.
