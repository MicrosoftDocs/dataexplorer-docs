...
> [!IMPORTANT]
> When specifying that the creation time should be overridden, make sure the `Lookback` property in the target table's effective [Extents merge policy](kusto/management/mergepolicy.md) is aligned with the values in your blob or file paths.
