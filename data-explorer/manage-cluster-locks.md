...
As an administrator, you can lock your cluster to prevent accidental deletion of data. The lock overrides any user permissions set by using [Azure Data Explorer role-based access control](/kusto/access-control/role-based-access-control?view=azure-data-explorer&preserve-view=true).
...
1. Select **Add**.
1. Enter a name and lock level for the lock. Optionally, add notes that describe the lock.
...
To delete a lock, select the **Delete** button in the row where the lock appears.
