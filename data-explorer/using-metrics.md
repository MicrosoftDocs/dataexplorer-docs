...
| Batches processed | Count | Sum, Max, Min | Number of batches completed for ingestion. <br> `Batching Type`: whether completion of batch was based on batching time, data size, or number of files limit, as set by [batching policy](./kusto/management/batchingpolicy.md). | Database, Batching Type |
...
| Ingestion result  | Count | Sum | Total number of sources that either failed or succeeded to be ingested.<br> <br> Use **apply splitting** to create buckets of success and failure results and analyze the failures categories (**Value** > **Status**). <br>For more information about possible fail categories, see [Ingestion error codes in Azure Data Explorer](error-codes.md). <br><br>Note:<br>- Using Event Hub or IoT Hub ingestion events are pre-aggregated into one blob, and then treated as a single source to be ingested. Therefore, pre-aggregated events appear as a single ingestion result after pre-aggregation.<br>- Transient failures are retried internally to a limited number of attempts. Each transient failure is reported as a transient ingestion result. Therefore, a single ingestion may result with more than one ingestion result. | Status |
