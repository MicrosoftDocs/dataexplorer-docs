...
| Queue length | Count | Avg | Number of pending messages in a component's input queue. In the Batching Manager component - there is a message per blob. In the Ingestion Manager component - there is a message per batch (batch is a single ingest command with one or more blobs)  | Component Type |
