...
|`hint.remote`|string||This hint is only relevant for cross cluster queries. Setting the value to `local` forces running the Python script on the local cluster. Use it in case the Python plugin is disabled on the remote cluster.|
