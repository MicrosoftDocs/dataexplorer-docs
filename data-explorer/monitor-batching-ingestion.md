...
In the *batching ingestion* process, Azure Data Explorer (ADX) optimizes [data ingestion](ingest-data-overview) for high throughput by batching the incoming data into small chunks based on a configurable [ingestion batching policy](kusto/management/batchingpolicy.md). The small batches of incoming data are then merged and optimized for fast query results. These optimizations make batching ingestion the most performant of the [methods for ingesting data](ingest-data-overview#batching-vs-streaming-ingestion) used by ADX.
...
ADX provides a set of [ingestion metrics](using-metrics#ingestion-metrics) in the Azure Monitor in Azure portal. These metrics can give you detailed information about the ingestion result, the amount of ingested data, and the latency of the ingestion, for each of the batching ingestion stages and components.
...
## Create metric charts in Azure Monitor

Use the following steps to create metric charts in the Azure Monitor in Azure portal:
...
1. To create a metric chart, select the **Metric** name and the relevant **Aggregation** per metric. In this article we'll use the **Aggregation** values *sum* and *avg*.
...
1. Use the time selector to change the time range for the chart. In this article we'll use the value **Last 48 hours**.
...
### Set the Scope and Metric Namespace
...
In this example, we'll use this metric to see the result of our ingestion attempts, including detailed status information to help troubleshoot any failed attempts.
...
1. Select the **Apply splitting** button and choose the **Component Type** dimension to segment the chart by the different ingestion components.
...
1. Select the **Apply splitting** button and choose **Component Type** to split the **Blob Received** metric by component type.
...
The metrics **Stage Latency** and **Discovery Latency** monitor latency in the ingestion process and tell you whether any long latency is occurring in ADX or before data arrives in ADX for ingestion.
...
1. Select the **Apply splitting** button and choose the **Batching Type** dimension to segment the chart based on the reason the batch was sealed. Batches are sealed when the batch reaches the limits set in the batching policy for the batching time, data size, or number of files.
1. Select the **Add filter** button and filter on the blobs sent to the *GitHub* database. After selecting the filter values, click away from the filter selector to close it.

The chart shows the number of sealed batches with data sent to the *GitHub* database over time, split by the **Batching Type**.

:::image type="content" source="media/monitor-batching-ingestion/batches-processed-by-batching-type-chart.png" alt-text="Screenshot of the Metrics pane in Azure portal showing a chart of discovery latency for ingestion from the github database aggregated by avg and split by component type.":::

* Notice that there are 2-4 batches per time unit over time, and that all batches are split by time as estimated in the stage latency section where you can see that it takes around 5 minutes to batch data based on the default batching policy.

### Batch size, duration, and blob count

Now let's look in more detail at the size, speed, and efficiency of the how the batches are being processed.
...
1. Select the **Add filter** button above the chart and choose the **Component Name** property to filter the events received by a specific data connection defined on your cluster. 
1. In this example, we filter on the **GitHubStreamingEvents** data connection by selecting *Component* as the **Property**, *=* as the **Operator**, and *GitHubStreamingEvents** in the **Values** drop-down. After selecting the filter values, click away from the filter selector to close it.
...
:::image type="content" source="media/monitor-batching-ingestion/events-processed-events-dropped-chart.png" alt-text="Screenshot of the Metrics pane in Azure portal showing a chart with graphs of the events processed and events dropped during ingestion from the github database aggregated over time.":::
...
1. Select **Scope** and in the **Select a scope** dialog, search for the namespace of the Event Hub that sent data to your data connection and select it.
