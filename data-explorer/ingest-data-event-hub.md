...
1. To see the content of the messages, run the following query:
    ...
    > [!NOTE]
    > * Azure Data Explorer has an aggregation (batching) policy for data ingestion, designed to optimize the ingestion process. The default policy is configured to 5 minutes as the maximum delay time, 1000 items or total size of 1G for batching, so you may experience a latency. For more information see [batching policy](kusto/management/batchingpolicy.md). 
