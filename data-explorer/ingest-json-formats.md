...
> [!NOTE]
> When ingesting using the [ingestion wizard](ingest-data-wizard.md), select **Ignore data format errors**, to ingest data in JSON format, thus ignoring ill-formatted rows errors (only the bad line will be ignored, the rest of the file will be ingested fully). If you leave this check box unselected, the data will be ingested in multijson format. For more information, see [edit the schema in the ingestion wizard](ingestion-wizard-existing-table.md#edit-the-schema).
