...
> [!NOTE]
> When ingesting using the [ingestion wizard](ingest-data-wizard.md), the default format is `multijson`. The format can handle multiline JSON records and arrays of JSON records. When a parsing error is encountered, the entire file is discarded. If you want to be able to handle records that are not well-formed, you can select the option to "Ignore data format errors.", which will switch the format to `json` (JSON Lines).
> 
