...
The plugin supports managed (streaming) and queued (batching) [ingestion](ingest-data-overview.md#queued-vs-streaming-ingestion). The default ingestion type is *queued*.
...
There are multiple ways to flatten dynamic columns by using the [extend](kusto/query/extend-operator.md) operator or [bag_unpack()](kusto/query/bag-unpackplugin.md) plugin. You can use either of them in the update policy *Transform_TargetTableName()* function.
