...
* **Use an [update policy](/kusto/management/update-policy)**: Transform dynamic data type columns using an update policy. We recommend this approach for querying large volumes of data.
...
There are multiple ways to flatten dynamic columns by using the [extend](/kusto/query/extend-operator) operator or [bag_unpack()](/kusto/query/bag-unpack-plugin) plugin. You can use either of them in the update policy *Transform_TargetTableName()* function.
