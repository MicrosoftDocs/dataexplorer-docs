...
* **New and improved shard storage format.** This storage format is a compressed column store with special attention to unstructured (text) and semi-structured data types. This format incorporates special encoding and compression for numeric types and timestamps, dictionary encoding for low cardinality text fields, and a layout optimized for analytical queries. Short storage is augmented with redesigned indexes for all fields, with an inverted index for text and JSON data, and range index for numerical data. This new index is granular, allowing many queries to evaluate parts of the query to be based on the index without scanning the data.
* **Redesign of the low-level shard query engine.** The entire shard query is just-in-time compiled into highly efficient machine code, resulting in a fast and efficient fused query evaluation logic. Query compilation is guided by the data statistics gathered from all the shards, and tailored to the specifics of the column encoding.
