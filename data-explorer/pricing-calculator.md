...
* **Hot cache retention (days)**: Period for which your data is stored in cache for fast query access. Ingested data that's cached according to our [cache policy](/kusto/management/cache-policy) on the local SSD of the Engine service. Your query performance requirement determines the amount of compute nodes and local SSD storage needed.
