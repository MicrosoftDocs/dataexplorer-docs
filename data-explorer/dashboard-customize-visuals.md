...
To render plotly visual the query should generate a table containing a single string cell containing [Plotly JSON](https://plotly.com/chart-studio-help/json-chart-schema/). This Plotly JSON string can be generated by either
- Creating it dynamically in Python using [plotly package](https://plotly.com/python/getting-started/), to be run by the [python() plugin](kusto/query/pythonplugin.md).
- Retrieving it from a table that stores pre-cooked Plotly JSON templates and just update the required data fields using KQL string manipulation functions.
