...
If your cluster already has the desired managed identity assigned to it, copy its object ID by using the following steps:

1. Sign in to the [Azure portal](https://portal.azure.com/) by using an account associated with the Azure subscription that contains your cluster.
...
1. In the Azure portal, go to the storage account that contains the data you want to ingest.
...
> [!NOTE]
> Granting **Owner** or **Contributor** permissions isn't sufficient and causes the ingestion to fail.
...
> [!IMPORTANT]
> If network problems occur, Azure Storage might return a `Download Forbidden` error.
> This error might happen if you use a private link to access your storage account.
> In such cases, if the permissions are correct, check the connectivity to your storage account.
...
You can define the usage Managed Identity policy at the cluster or database level of the target cluster.
...
> [!IMPORTANT]
>
> * When you use managed identities to ingest data by using the C# SDK, you must provide a blob size in `StorageSourceOptions`. If you don't set the size, the SDK attempts to fill in the blob size by accessing the storage account, resulting in a failure.
> * The *size* parameter should be the raw (uncompressed) data size, not the blob size.
> * If you don't know the size at the time of ingestion, specify a value of zero (0). The service attempts to discover the size by using the managed identity for authentication.
