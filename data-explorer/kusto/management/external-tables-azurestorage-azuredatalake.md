...
| `sampleUris`     | `bool`   | If set, the command result provides several examples of simulated external data files URI as they are expected by the external table definition. This option helps validate whether *[Partitions](#partitions)* and *[PathFormat](#path-format)* parameters are defined properly. |
| `filesPreview`   | `bool`   | If set, one of command result tables contains a preview of [.show external table artifacts](#show-external-table-artifacts) command. Like `sampleUri`, the option helps validate *[Partitions](#partitions)* and *[PathFormat](#path-format)* parameters of external table definition. |
...
| `dryRun` | `bool` | If set, the external table definition is not persisted. This option is useful for validating external table definition, especially in conjunction with `filesPreview` or `sampleUris` parameter. |
