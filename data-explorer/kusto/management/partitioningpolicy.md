...
>   * However, every additional table adds overhead to the background data partitioning process that runs on the cluster's nodes. Setting a policy on more tables will result with more cluster resources being used, and higher cost due to underlying storage transactions.
...
> [!NOTE]
> The data partitioning operation adds significant processing load. We recommend applying a hash partition key on table only under the following conditions:
