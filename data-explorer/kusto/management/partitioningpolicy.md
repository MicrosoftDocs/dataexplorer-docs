...
* If a hash partition key includes values which are significnatly more prevalent than others - for example, an empty string, a generic value (e.g. `null` or `N/A`), or they represent an entity (e.g. `tenant_id`)
...
* If a uniform range datetime partition key has a large enough percentage of values which are "far" from the majority of the values in the column (for example, datetime values from the distant
  past or future), that could increase the overhad of the data partitioning process, and lead to many small extents the cluster needs to keep track of.
