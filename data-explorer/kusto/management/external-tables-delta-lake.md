...
| `AutoUpdateSchema` | `bool`    | If set to `true`, the external table automatically tracks and reflects schema changes from the remote delta table. Requires a [query acceleration policy](query-acceleration-policy.md) to be enabled on the table. Only available when the schema is inferred (not user-provided). Default is `false`. |
