...
The user or application authenticates via AAD to Kusto, and the same token is then used to access the SQL Server network endpoint.
  ...
  * *Managed Identity authentication* can be used to access the sql resource by adding (`Authentication="Active Directory Managed Identity"`) for system assigned managed identity or (`Authentication="Active Directory Managed Identity";User Id={object_id}`) for user assigned managed identity with its object id.
...
> [!NOTE]
If the external table is used for [continuous export](data-export/continuous-data-export.md), authentication must be performed by UserName/Password or Managed Identities. 
