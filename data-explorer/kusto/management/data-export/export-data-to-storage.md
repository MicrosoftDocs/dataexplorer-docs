...
| `compressionType` | `string` | Indicates the type of compression to use. Possible values are `gzip`, and for `parquet` format `snappy`, `lz4_raw`, `brotli`, or `zstd` as well. Default is `gzip`. |
