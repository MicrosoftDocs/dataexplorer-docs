...
| `managedIdentity` | `string` | The managed identity on behalf of which the continuous export job will run. This can be an object ID, or the `system` reserved word. This is required to run Continuous export jobs exporting data to an external table that uses impersonation, and also if the `Query` argument references tables in other databases. For more information on managed identities, see [managed identity overview](../../../managed-identities-overview.md) |
