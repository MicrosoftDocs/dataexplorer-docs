...
* Continuous export supports cross-database calls only for dimension tables. All fact tables must reside in the local database. See more details in [Export from fact and dimension tables](#export-from-fact-and-dimension-tables). The continuous export must be configured with a [managed identity](continuous-export-with-managed-identity.md) if it includes cross database calls.
