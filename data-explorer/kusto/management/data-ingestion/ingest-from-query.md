...
|`persistDetails` |A Boolean value that, if specified, indicates that the command should persist the detailed results for retreival by the [.show operation details](../operations.md#show-operation-details) command. Successful results are also persisted. Defaults to `false`. |`with (persistDetails=true)`|
