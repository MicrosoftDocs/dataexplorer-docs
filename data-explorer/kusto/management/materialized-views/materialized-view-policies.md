...
* The policy is applied to the [materialized part](materialized-view-overview.md#how-materialized-views-work) of the view only.
  ...
  * If a row level security policy is defined on the source table and a user is not included in the source table's policy, querying the materialized view fails even if the user is included in the materialized view's own row level security policy. This is because the [delta part](materialized-view-overview.md#how-materialized-views-work) of the query accesses the source table directly, and the source table's row level security policy is enforced on that access. To resolve this, either add the user to the source table's row level security policy, or use the [materialized_view()](../../query/materialized-view-function) function to query only the materialized part of the view, which does not require access to the source table.
