...
* Manual / out of protocol changes to accelerated delta tables are not supported (for example manually editing _delta_log directory / files or Parquet data files). The feature assumes the delta table complies with the delta protocol and was created and altered through well-known delta operations.
