...
|||
|---|---| 
|`.show` `operations`              |Returns all operations that the cluster has processed and is processing
...
|NodeId |String |If the command has a remote execution (for exmaple, DataIngestPull) - NodeId will contain the ID of the executing remote node 
...
Operations can (optionally) persist their results, and the results can be retrieved when the operation is complete using the `.show` `operation` `details`.
