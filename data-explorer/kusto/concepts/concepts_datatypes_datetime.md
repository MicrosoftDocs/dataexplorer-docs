...
A `datetime` value is always in the UTC time zone.
...
|`datetime(null)`                                            |See [null values](./concepts_datatypes_null.md).              |
|`now()`                                                     |The current time.                                             |
|`now(`-*timespan*`)`                                        |`now()-`*timespan*                                            |
|`ago(`*timespan*`)`                                         |`now()-`*timespan*                                            |

`now()` and `ago()` indicate a `datetime` value compared with the moment
in time when Azure Log Analytics started to execute the query. These can appear multiple
