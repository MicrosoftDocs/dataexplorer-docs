...
* Reducing the result set size by modifying the query to not
   return uninteresting data. This is commonly useful when
   the initial (failing) query is too "wide" (for example, it does not
   project away data columns that are not needed.)
* Reducing the result set size by shifting post-query processing
   (such as aggregations) into the query itself. This is useful
   in scenarios where the output of the query is fed to another
   processing system which then performs additional aggregations. 
* Switching from queries to using [data export](../management/data-export/index.md).
   This is appropriate when you want to export large sets
...
* Using the [summarize operator](../query/summarizeoperator.md) group and aggregate over
...
* Using a [take operator](../query/takeoperator.md) to sample the query output.
* Using the [substring function](../query/substringfunction.md) to trim wide free-text columns.
* Using the [project operator](../query/projectoperator.md) to drop any uninteresting column
   from the result set.

One can disable result truncation by using the `notruncation` request
option. It's strongly recommended that, in this case, some form of
...
It is also possible to have more refined control over result truncation
...
you'll reach client limits which are currently not configurable. 
...
so we can prioritize a streaming client appropriately.
...
Kusto -- either for export purposes (in which case you are urged to
...
data set to 10%. The two queries below show how to perform this
sampling. The first is a statistical sampling (using a random number
generator). The second is deterministic sampling (by hashing
...
There is currently no switch to increase the maximum string set size.
As a workaround, rephrase the query to reduce the amount of data that
has to be buffered. For example, by projecting-away unneeded columns before
they "enter" operators such as join and summarize. Or, for example, by using the [shuffle query](../query/shufflequery.md) strategy.
...
By default, timeout is set to four minutes for queries and ten minutes for
...
when it is 100% the cluster will assign all CPUs on each node (for example, on a cluster
...
In case the tree depth exceeds an internal threshold (several thousand levels), the query is considered too complex for processing and will fail with an error code indicating the relational operators tree exceeds limits.
