...
| `request_impersonation_disabled` | `bool` | If set to `true`, the service will not allow using the caller's identity to access downstream services on behalf of the caller. Note this flag will also block cross-cluster joins. |
