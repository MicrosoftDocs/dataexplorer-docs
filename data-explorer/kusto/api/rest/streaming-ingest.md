...
|`x-ms-client-version`   |The (friendly) version identifier for the client making the request.                  | Optional, but we strongly recommend that you specify it, since it's required in some scenarios (such as canceling a running query), where it's used to identify the request |
