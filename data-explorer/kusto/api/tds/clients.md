...
```
It is possible to use service principal authentication via ODBC, but it requires providing AAD tenant ID via ODBC connection string. The AAD tenant ID should be specificed via ADX specific tuning option syntax in Language field of connection string. For example:
