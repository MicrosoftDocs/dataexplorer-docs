...
| PersistAfterEachRun     | Usually the web explorer will persist in the unload event. When hosting in **iframes**, it does not fire in some cases. This flag will then trigger **persisting local state** after each query run. Therefore, any data loss that occurs, will only affect text that had never been run, thus limiting its impact. | false         |
