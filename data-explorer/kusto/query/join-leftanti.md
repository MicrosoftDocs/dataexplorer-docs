...
The `leftanti` join flavor returns all records from the left side that don't match any record from the right side.
...
*LeftTable* `|` `join` `kind=leftanti` [ *Hints* ] *RightTable* `on` *Attributes*
