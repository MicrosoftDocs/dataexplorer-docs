...
* Record `R=3`, `from_TimelineColumn` = `2019-11-01`,  `to_TimelineColumn` = `2019-11-03`:
    ...
    * `dcount_new_values` – the number of users in 11/3 which weren’t seen in 11/1. This includes a single user – `5`. 
    * `dcount_retained_values` – out of all new users in 11/1, how many retained until 11/3? These are 3 (`[0,2,4]`), 
    while `count_churn_values` is 1 (user=`3`). 
    * `retention_rate` = 0.75 – the 3 retained users out of the 4 new users that were 1st seen in 11/1. 

* Record `R=9`, `from_TimelineColumn` = `2019-11-02`,  `to_TimelineColumn` = `2019-11-04`:
    * This record focuses on the new users that were first seen in 11/2 – users `1` and `5`. 
    * `dcount_new_values` – the number of users in 11/4 which weren’t seen through all periods `T0 .. from_Timestamp`. Meaning, 
    users which are seen in 11/4 but were not seen in either 11/1 and 11/2 – there are no such users. 
    * `dcount_retained_values` – out of all new users in 11/2 (`[1,5]`), how many retained until 11/4? There’s one such user (`[1]`), 
    while count_churn_values is 1 (user `5`). 
    * `retention_rate` is 0.5 – the single user that was retained in 11/4 out of the 2 new ones in 11/2. 
