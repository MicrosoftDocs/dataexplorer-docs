...
Partitioning queries like that would be easy for simple aggregates, such as count() and sum(). It is possible, however, to also perform them for more complex aggregates such as dcount() and percentiles(). This topic explains how Azure Log Analytics supports such calculations.
...
When having a too large datasets where we need to run a periodic queries over this dataset but running the regular queries to calculate [`percentile()`](query_language_percentiles_aggfunction.md) or [`dcount()`](query_language_dcount_aggfunction.md) over these big dateset hits Azure Log Analytics limits.
To solve this problem, Newly added data may be added to a temp table as hll or tdigest values using [`hll()`](query_language_hll_aggfunction.md) when the required operation is dcount or [`tdigest()`](query_language_tdigest_aggfunction.md) when the required operation is percentile using `set/append` or `update policy`, In this case, the intermediate results of dcount or tdigest are saved into another dataset which should be smaller than the target big one.
Then when we need to get the final results of these values, the queries may use hll/tdigest mergers: [`hll_merge()`](query_language_hll_merge_aggfunction.md)/[`merge_tdigests()`](query_language_merge_tdigests_aggfunction.md), Then, After getting the merged values, [`percentile_tdigest()`](query_language_percentile_tdigestfunction.md) / [`dcount_hll()`](query_language_dcount_hllfunction.md) may be invoked on these merged values to get the final result of dcount or percentiles.
...
When using the functions of [`hll()`](query_language_hll_aggfunction.md), [`hll_merge()`](query_language_hll_merge_aggfunction.md) and [`dcount_hll()`](query_language_dcount_hllfunction.md), the equivalent query for above will end after ~1.3 seconds and shows that hll functions speeds up 
...
Note: the results of the queries are not 100% accurate due to the error of the hll functions.(see [`dcount()`](query_language_dcount_aggfunction.md) for further information about the errors).
