...
|ARM resource ID|`{ armResourceId: <id> }`|Provide the ID in the `armResourceId` field of the *Options* argument.|
|Azure AD access token|`{ token: <token> }`|Provide the token in the `token` field of the *Options* argument.|
