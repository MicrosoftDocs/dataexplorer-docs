...
> * If you need to persist a combined hash, it's recommended to use [hash_sha256()](sha256hash-function.md), [hash_sha1()](sha1-hash-function.md), or [hash_md5()](hash-md5-function.md) and combine the hashes with a [bitwise operator](bin-operators.md). These functions are more complex to calculate than `hash()`.
