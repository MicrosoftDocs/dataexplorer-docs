...
* *SearchPredicate*: A mandatory predicate that defines what to search for (in other words,
  ...
  The syntax for *SearchPredicate* extends and modifies the normal Azure Log Analytics syntax for Boolean expressions:
  ...
  **Other Boolean expressions**: All regular Azure Log Analytics Boolean expressions are supported by the syntax.
...
**Unlike** the [find operator](query_language_findoperator.md), the `search` operator does not support the following:
