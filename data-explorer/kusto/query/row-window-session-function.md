...
* *Expr* is an expression whose values are to be groups together in sessions.
...
* *Restart* is an optional scalar expression of type `boolean`. If specified,
  every value that evaluates to `true` will immediately have the session restarted.
...
2. For every value, determine if it establishes a new sesion or not.

3. If it establishes a new session, emit the value of *Expr*. Otherwise, emit
   the previous value of *Expr*.

The condition the determines if the value represents a new session or not is
...
a session cannot exceed 1 hour, and it continues as long as records are less than
5 minutes apart.
