...
|*InferenceOptions*|`dynamic`||Additional inference options, currently the only supported option is `UseFirstRowAsHeader` that is applicable for the delimited file formats. For example `'InferenceOptions': {'UseFirstRowAsHeader': true}`  
