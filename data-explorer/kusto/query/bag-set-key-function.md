...
| *key* | string | &check; | The key to set. Either a JSON path (you can specify a key on the nested levels using [JSONPath](jsonpath.md) notation) or if you want to set a key that's in the first level, then you can just provide its name, e.g. "name". Array indexing or root JSON path aren't supported. |
