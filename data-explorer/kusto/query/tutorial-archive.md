...
Achieve the same result by using [sort](./sort-operator.md), and then [take](./take-operator.md).
...
## Summarize using scalar values

Use scalar (numeric, time, or interval) values in the `by` clause by putting the values into bins, using the [bin()](./bin-function.md) function.
...
Use [let](./let-statement.md) to separate out the parts of the query expression in the preceding `join` example. The results remain unchanged.
