...
Our sample database has a table called `StormEvents`. To find out how large the table is, we'll pipe its contents into an operator that counts the rows in the table.
...
Use [project](./project-operator.md) to select specific columns. See the following examples, which use both the [project](./project-operator.md)
...
To see a random sample of five rows:
...
|---|---|---|---|---|
|2007-09-18 20:00:00.0000000|2007-09-19 18:00:00.0000000|Heavy Rain|FLORIDA 9 inches of rain fell in a 24-hour period across parts of coastal Volusia County.|
|2007-09-20 21:57:00.0000000|2007-09-20 22:05:00.0000000|Tornado|FLORIDA|A tornado touched down in the town of Eustis at the northern end of West Crooked Lake. The tornado quickly intensified to EF1 strength, as it moved north northwest through Eustis. The track was just under two miles long, and had a maximum width of 300 yards.  The tornado destroyed 7 homes. 27 homes received major damage and 81 homes reported minor damage. There were no serious injuries. Property damage was set at $6.2 million.|
...
[take](./take-operator.md) shows rows from the table in no particular order, so let's sort them. 
...
[Scalar expressions](./scalar-data-types/index.md) can include all of the usual operators (`+`, `-`, `*`, `/`, `%`). A range of useful functions are available.

## Aggregate groups of rows: *summarize*

To count the number of events that occur in each state:
...
[summarize](./summarize-operator.md) groups together rows that have the same values in the `by` clause, and then uses an aggregation function (for example, `count`) to combine each group into a single row. In this case, there's a row for each state, and a column for the count of rows in that state.

A range of [aggregation functions](aggregation-functions.md) are available. You can use several aggregation functions in one `summarize` operator, to produce several computed columns. For example, you can get the count of storms per state, and the sum of unique types of storm per state. Then, you can use [top](./top-operator.md) to get the most storm-affected states:
...
* Each column is named in `summarize`.
...
*Syntax note*: The [bin()](./bin-function.md) is the same as the floor() function in many languages. It reduces every value to the nearest multiple of the modulus that you supply, so that [summarize](./summarize-operator.md) can assign the rows to groups.
...
*Syntax note*: Technically, `render` is a feature of the client, rather than a part of the query language. Still, it's integrated into the language, and is useful for envisioning your results.
...
To see how activity varies over the average day, count events by the time modulo one day, binned into hours:
...
Currently, `render` doesn't label durations properly. You can use `| render columnchart` instead:
...
To find two specific event types and in which state each of them happened, you can pull storm events with the first `EventType` and the second `EventType`, and then join the two sets on `State`:
...
To find out how long each user session lasts, you can use `extend` to provide an alias for the two timestamps, and then compute the session duration:
...
It's recommended to use `project` to select only the relevant columns before you perform the join. In the same clause, you can rename the `timestamp` column.

## Plot a distribution

Returning to the `StormEvents` table, to find out how many storms there are of different lengths:
...
To find out what ranges of durations there are in different percentages of storms, use the preceding query from [Plot a distribution](#plot-a-distribution), but replace `render` with:
...
To calculate the percentage of direct injuries, from all injuries:
...
*Tip*: In Kusto Explorer, to execute the entire query, don't add blank lines between parts of the query.
