...
`percentiles_array()` works like [percentiles()](percentiles-aggfunction.md) to calculate percentiles, and `percentilesw_array()` works like [percentilesw()](percentiles-aggfunction.md) to calculate weighted percentiles. However, instead of returning the values in individual columns, they return them in a single column of dynamic array type.
