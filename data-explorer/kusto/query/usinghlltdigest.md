...
To solve this problem, Newly added data may be added to a temp table as hll or tdigest values using [`hll()`](hll-aggfunction.md) when the required operation is dcount or [`tdigest()`](tdigest-aggfunction.md) when the required operation is percentile using [`set/append`](../management/data-ingest.md) or [`update policy`](../concepts/updatepolicy.md), In this case, the intermediate results of dcount or tdigest are saved into another dataset which should be smaller than the target big one.
