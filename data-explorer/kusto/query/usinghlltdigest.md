...
> [!WARNING]
> The precise binary representation of  hll() and tdigest() results is undocumented and may change over time. There’s no guarantee that these functions return the same output for the same input, and one should not take a dependency on them.
