---
title: Partitioning and composing intermediate results of aggregations - Azure Data Explorer
...
To solve this problem, newly added data may be added to a temp table as `hll` or `tdigest` values using [`hll()`](hll-aggfunction.md) when the required operation is `dcount` or [`tdigest()`](tdigest-aggfunction.md) when the required operation is percentile using [`set/append`](../management/data-ingestion/index.md) or [`update policy`](../management/updatepolicy.md). In this case, the intermediate results of `dcount` or `tdigest` are saved into another dataset, which should be smaller than the target large one.
...
The Retention Query.
Assuming you have a table that summarizes when each Wikipedia page was viewed (sample size is 10M), and you want to find for each date1 date2
