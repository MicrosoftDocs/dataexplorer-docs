...
Suppose that you want to calculate the count of distinct users over the last seven days, every day. One way to do it would be to run `summarize dcount(user)` once a day with a span filtered to the last seven days. This query is inefficient, because each time the calculation is run there's a six-day overlap with the previous calculation. Another option is to calculate some aggregate for each day, and then combine these aggregates in an efficient way. This option requires you to "remember" the last six results, but it's much more efficient.

Partitioning queries like that would be easy for simple aggregates, such as count() and sum(). It's possible, however, to also do them for more complex aggregates, such as `dcount()` and `percentiles()`. This topic explains how Kusto supports such calculations.

The following examples show how to use the algorithm functions for [HyperLogLog (`hll`)](hll-aggfunction.md) and [T-digest (`tdigest`)](tdigest-aggfunction.md) for aggregation. These examples demonstrate that using these functions in some scenarios may greatly improve performance:
...
For example, when persisting the output of `hll` function with accuracy level 4, the size of the hll object exceeds the default MaxValueSize, which is 1 MB.
...
To avoid this issue, use the special encoding policy type `bigobject`, which overrides the MaxValueSize to 2 MB like this:
...
Assume there's a table, PageViewsHllTDigest, which has the HLL values over Pages viewed in each hour. The aim is to get these values, but binned to `12h`, to merge the HLL values using the hll_merge() aggregate function by timestamp binned to `12h`. Then, call the function dcount_hll to get the final distinct count:
...
Kusto limits are reached with datasets that are too large, where you need to run periodic queries over the dataset, but run the regular queries to calculate [`percentile()`](percentiles-aggfunction.md) or [`dcount()`](dcount-aggfunction.md) over these large datasets.

::: zone pivot="azuredataexplorer"

To solve this problem, newly added data may be added to a temp table as HLL or T-Digest values using [`hll()`](hll-aggfunction.md) when the required operation is [`dcount()`](dcount-aggfunction.md) or [`tdigest()`](tdigest-aggfunction.md) when the required operation is percentile using [`set/append`](../management/data-ingestion/index.md) or [`update policy`](../management/updatepolicy.md), In this case, the intermediate results of `dcount()` or `tdigest()` are saved into another dataset, which should be smaller than the target big one.

::: zone-end

::: zone pivot="azuremonitor"

To solve this problem, newly added data may be added to a temp table as HLL or T-Digest values using [`hll()`](hll-aggfunction.md) when the required operation is [`dcount()`](dcount-aggfunction.md). In this case, the intermediate results of `dcount()` are saved into another dataset, which should be smaller than the target big one.

::: zone-end

When you need to get the final results of these values, the queries may use hll/tdigest mergers: [`hll-merge()`](hll-merge-aggfunction.md)/[`tdigest_merge()`](tdigest-merge-aggfunction.md), Then, after getting the merged values, [`percentile_tdigest()`](percentile-tdigestfunction.md) / [`dcount_hll()`](dcount-hllfunction.md) may be invoked on these merged values to get the final distinct count or percentiles.
...
If you save the HLL and T-Digest values (which are the intermediate results of dcount and percentile) into a temp table, PageViewsHllTDigest, using an update policy or set/append commands, you may only merge the values and then use dcount_hll/percentile_tdigest using the following query:
