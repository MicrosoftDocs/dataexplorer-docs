...
- In some cases the dynamic objects which are generated by the `hll` or the `tdigest` aggregate functions may be big and exceeds the default MaxValueSize property in the encoding policy. In this case, the object will be ingested as null.
...
Trying to ingest this into a table before applying this kind of policy will ingest null:
...
in order to avoid this, one can use a special [encoding policy type](../management/encoding-policy.md#alter-column-encoding-policy-by-type) `bigobject` which overrides the MaxValueSize to 2MB like this :
