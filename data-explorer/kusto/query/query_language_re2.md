...
There are a few functions in Azure Log Analytics perform string matching, selection, and extraction
...
- [matches regex](concepts_datatypes_string_operators.md)
...
- [trimend()](query_language_trimendfunction.md)
- [trimstart()](query_language_trimstartfunction.md)

The regular expression syntax supported by Azure Log Analytics is that of the
[re2 library](https://github.com/google/re2/wiki/Syntax), and is
detailed below. Note that these expressions must be encoded in
Azure Log Analytics as `string` literals, and all of Azure Log Analytics's string quoting rules
apply. For example, the regular expression `\A` matches
the beginning of a line (see the table below), and is specified
in Azure Log Analytics as the string literal `"\\A"` (note the "extra" backslash (`\`)
character.)
