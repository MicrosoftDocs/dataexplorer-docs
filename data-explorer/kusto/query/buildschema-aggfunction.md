...
> [!TIP]
> If a syntax error occurs with `buildschema(`*json_column*`)`, check that the column type is `dynamic`. To convert a JSON `string` into a `dynamic` value, use the [parse_json()](parsejsonfunction.md) function.
