...
By default, Azure Log Analytics indexes all columns, including columns of type `string`.
...
term. By default, Azure Log Analytics breaks each `string` value into maximal sequences of
alphanumeric characters, and each of those is made into a term. For example,
in the following `string`, the terms are `Azure`, `WilliamGates3rd`, and
the following substrings: `ad67d136`, `c1db`, `4f9f`, `88ef`, `d94f3b6b0b5a`:

```
Azure:  ad67d136-c1db-4f9f-88ef-d94f3b6b0b5a;;WilliamGates3rd
```

By default, Azure Log Analytics builds a term index consisting of all terms that are
...
operator for example, Azure Log Analytics will revert to scanning the values in the column
if it cannot determine a match, which is far slower than looking the term
up in the term index.)

It is possible to modify the way Azure Log Analytics breaks a `string` value into terms
