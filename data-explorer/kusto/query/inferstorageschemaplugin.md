...
|*Mode*|string||The schema inference strategy. A value of: `any`, `last`, `all`. The functon will infer data schema from the first found file, from the last written file, or from all files respectively. The default value is `last`.|
