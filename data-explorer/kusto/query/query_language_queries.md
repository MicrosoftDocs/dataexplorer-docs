...
A query is a read-only operation against an Azure Log Analytics Engine cluster's
...
As ad-hoc query of data is the top-priority scenario for Azure Log Analytics,
the Azure Log Analytics Query Language syntax is optimized for non-expert users
...
(references to Azure Log Analytics tables) and one or more **query operators** applied
...
[scalar expressions](./query_language_scalars.md) over the columns of the input.
