...
* *aggregation function*: (optional) aggregates multiple rows in the input table to a single row in the output table. Currently supported funcitons: `sum()`, `dcount()`, and `count()` (default is `count()`).
