...
ADD MENTION ABOUT LOGIC, EVALUATING LAST TO FIRST, FOR MORE INFO SEE X
...
### Sessions tagging XXX ASK ROY HOW SESSION-ID GETS INCREMENTED
...
Each record from the input is evaluated against all of scan’s steps from last to first. When a record *r* is considered against some step *s_k*, the following logic is applied:
...
* If the record *r* satisfies the condition of *s_k* using the state of the previous step *s_(k-1)*, then the following happens:
    ...
    1. The state of *s_(k-1)* becomes the state of *s_k* (the state of *s_(k-1)* is now empty)
    1. All the assignments of *s_k* are calculated and extend *r*
    1. The extended *r* is added to the output and to the state of *s_k*
* If *r* did not satisfy the condition of *s_k* with the state of *s_(k-1)*, *r* is checked with the state of *s_k*. If it satisfies the condition of *s_k* with the state of *s_k*, the following happens:
...
* If r does not satisfy the condition *s_k* with the state *s_k*, evaluate *s_k-1* and repeat the logic above. XXX VERIFY WITH ROY