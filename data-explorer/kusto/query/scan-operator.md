...
For instance, if a scan operator has *n* steps named *s_1*, *s_2*, ..., *s_n* then step *s_k* would have *k* records in its state corresponding to *s_1*, *s_2*, ..., *s_k*. For a detailed example, see the [scan logic walkthrough](#scan-logic-walkthrough).
