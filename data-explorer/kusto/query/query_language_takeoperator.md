...
by the query (no `take` operator is used), Azure Log Analytics limits that number by default.
Please see Azure Log Analytics query limits for details.
...
Azure Log Analytics does not support the complementary `skip` operator. This is intentional, as
