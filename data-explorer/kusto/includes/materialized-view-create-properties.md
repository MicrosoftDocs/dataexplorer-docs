...
| lookback | `timespan` | Time span limiting the period of time in which duplicates are expected. See [Lookback period](../management/extents-overview.md#Lookback-period). |
| lookback_column | `string` | A `datetime` column in the view which serves as the reference for the lookback period. See [Lookback period](../management/extents-overview.md#Lookback-period). |
