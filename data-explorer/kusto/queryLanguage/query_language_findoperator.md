...
* *Predicate*:  A `boolean` [expression](../concepts/concepts_datatypes_bool.md) over the columns of the input tables *Table* [`,` *Table*, ...]. It is evaluated for each row in each input table. 
...
## Predicate Syntax
Please refer to the [where operator](./query_language_whereoperator.md) for some filtering functions summery.
...
* `find` scope can not include functions. To include a function in the find scope - define a [let statement](./query_language_letstatement.md) with [view keyword](./query_language_letstatement.md)

## Performance Tips
* Use tables as opposed to [tabular expressions](./query_language_tabularexpressionstatements.md)- in case of tabular expression the find operator falls back to a `union` query which can result in degraded performance
...
The next query finds all rows from all tables in which any column includes the word `Azure`.
The resulting records are transformed according to the [output schema](#output-schema). 

```
find "Azure Log Analytics"
```

## Term lookup across all tables matching a name pattern
The next query finds all rows from all tables whose name starts with `K`, and in which any column includes the word `Azure`.
The resulting records are transformed according to the [output schema](#output-schema). 

```
find in (K*) where * has "Azure"
```
...
### Use abbreviated notation to search across all tables
