---
title: t_test_unpaired_fl() - Azure Data Explorer
...
The function `t_test_unpaired_fl()` performs the [Unpaired t-Test](https://en.wikipedia.org/wiki/Student%27s_t-test#Unpaired_and_paired_two-sample_t-tests).

> [!NOTE]
> * `t_test_unpaired_fl()` is a [UDF (user-defined function)](../query/functions/user-defined-functions.md). For more information, see [usage](#usage).
...
`T | invoke t_test_unpaired_fl()(`*data1*`,` *data2*`,` *test_statistic*`,`*p_value*`,` *equal_var*`)`
...
`t_test_unpaired_fl()` is a user-defined [tabular function](../query/functions/user-defined-functions.md#tabular-function), to be applied using the [invoke operator](../query/invokeoperator.md). You can either embed its code in your query, or install it in your database. There are two usage options: ad hoc and persistent usage. See the below tabs for examples.
...
```kusto
let t_test_unpaired_fl = (tbl:(*), data1:string, data2:string, test_statistic:string, p_value:string, equal_var:bool=true)
{
...
| invoke t_test_unpaired_fl('sample1', 'sample2', 'test_stat', 'p_val')
```
...
<!-- csl: https://help.kusto.windows.net:443/Samples -->
```kusto
.create-or-alter function with (folder = "Packages\\Stats", docstring = "Unpaired t-Test")
t_test_unpaired_fl(tbl:(*), data1:string, data2:string, test_statistic:string, p_value:string, equal_var:bool=true)
{
    let kwargs = pack('data1', data1, 'data2', data2, 'test_statistic', test_statistic, 'p_value', p_value, 'equal_var', equal_var);
    let code =
        'from scipy import stats\n'
