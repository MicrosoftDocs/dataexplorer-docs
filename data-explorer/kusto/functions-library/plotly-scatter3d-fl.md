...
The function `plotly_scatter3d_fl()` creates an interactive 3D scatter chart to be rendered in [ADX dashboard](../../azure-data-explorer-dashboards.md) by customizing a pre-prepared [plotly](https://plotly.com/python/) template.
The function accepts a table containing the records to be rendered, the names of the x, y, z & aggregation columns and the chart title string, and returns a single cell table containing [plotly JSON](https://plotly.com/chart-studio-help/json-chart-schema/) that can be rendered in a dashboard tile as explained [here](../../dashboard-customize-visuals.md#plotly-preview).
