...
The function `log_reduce_predict_fl()` summarizes the number of appearances of each pattern in semi structured data. The patterns are retrieved from a pre-trained model, generated by `log_reduce_train_fl()`.
...
~~~kusto
let log_reduce_predict_fl=(tbl:(*), models_tbl: (name:string, timestamp: datetime, model:string), model_name:string, reduce_col:string, anomaly_str: string = "ANOMALY")
{
    let model_str = toscalar(models_tbl | where name == model_name | top 1 by timestamp desc | project model);
    let kwargs = bag_pack('logs_col', reduce_col, 'output_patterns_col', "LogReduce",'output_parameters_col', "", 'model', model_str, "anomaly_str", anomaly_str, 'output_type', "Summary");
    let code = ```
if 1:
...
BGL_logs
| take 100000 
| invoke log_reduce_predict_fl(models_tbl=ML_models, model_name="BGL", reduce_col="data")
~~~
...
log_reduce_predict_fl(tbl:(*), models_tbl: (name:string, timestamp: datetime, model:string), model_name:string, reduce_col:string, anomaly_str: string = "ANOMALY")
{
    let model_str = toscalar(models_tbl | where name == model_name | top 1 by timestamp desc | project model);
    let kwargs = bag_pack('logs_col', reduce_col, 'output_patterns_col', "LogReduce",'output_parameters_col', "", 'model', model_str, "anomaly_str", anomaly_str, 'output_type', "Summary");
    let code = ```
